...
function Checkout({ cart, setCart }) {
  ...
  useEffect(() => {
    ...
    if (loggedInUser) {
      setCustomerInfo({
        ...
        address: '', // Address can be filled by the user
        phone: '' // Phone can be filled by the user
      });
    } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const loggedInUser = JSON.parse(localStorage.getItem('user'));
      if (!loggedInUser?.id) throw new Error('User ID is not available.');
        ...
        <div className="row">
          ...
          <div className="col-md-6 mb-3">
            <input
              ...
              className="form-control"
              disabled // Disable editing for logged-in user's email
            />
          </div>
          <div className="col-md-6 mb-3">
            <input
